...
public class TestSearchAndReplaceInterceptor {
  ...
  public void testCsvToJson() throws Exception {
    final String input = "46038;辽宁链家校园;1499166866268; ;aaa".replaceAll(";","\t");
    final String output = "{'k1':'46038','k2':'辽宁链家校园','k3':'1499166866268','k4':' ','k5':'aaa'}";
    Context context = new Context();
    context.put("searchPattern", "(.+)\\t(.+)\\t(.+)\\t(.+)\\t(.+)");
    context.put("replaceString", "{'k1':'$1','k2':'$2','k3':'$3','k4':'$4','k5':'$5'}");
    testSearchReplace(context, input, output);
    //testSearchReplace(context, input, output);
  }
}